...
class NukiDevice extends Homey.Device {
  ...
  onInit () {
    ...
    this.homey.flow.getConditionCard('alarm_battery_keypad_opener')
      .registerRunListener(async (args, state) => {
        if (this.hasCapability('alarm_battery_keypad_opener')) {
          return args.device.getCapabilityValue('alarm_battery_keypad_opener');
        }
        else {
          return false;
        }
